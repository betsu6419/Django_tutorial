        ...
        for k in range(len(x_label)):
            ...
            for j in stock_data:
                if j.category == category:
                    if j.date.strftime('%Y/%m/%d')[:10]==x_label[k]:   
                        ln.append(j.money)
            if(len(ln) != 0):
...
def show_learn_grahp(request):
   ...
   #月毎＆カテゴリ毎の合計金額データを生成する。
    ...
    date_list2=[]
    for i in stock_data:
        date_list2.append((i.date))
    redays= list(set(date_list2))
    redays.sort(reverse=False)

    for category in category_list:
        mass=[]
        for i,j,k in matrix_list:
            if(j == category):
                mass.append(k)
        ...
        y = lr.predict(x)
        j=0
        firstday=date_list2[0]
        while y[j]>0:
            dayj=firstday+datetime.timedelta(days=j)
            matrix_list2.append([dayj.strftime('%Y/%m/%d')[:10],category,y[j]])
            j+=1
        deadline_i = firstday+datetime.timedelta(days=j)
        deadline.append([category,deadline_i.strftime('%Y/%m/%d')[:10] ])   
     #折れ線グラフのボーダーカラー色の設定       
    border_color_list=['254,97,132,0.8','54,164,235,0.8','0,255,65,0.8','255,241,15,0.8',\
                       '255,94,25,0.8','84,77,203,0.8','204,153,50,0.8','214,216,165,0.8',\
                       '33,30,45,0.8','52,38,89,0.8']
